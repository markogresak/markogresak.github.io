...
const COMPILED_FILE = (postId) => path.join(appRoot.path, "scripts", `compiled-${postId}.mjs`);
const validatePost = (value) => {
  var _a, _b, _c;
  return [
    {
      key: "meta.date",
      ok: typeof ((_a = value == null ? void 0 : value.meta) == null ? void 0 : _a.date) === "string"
    },
    {
      key: "meta.description",
      ok: typeof ((_b = value == null ? void 0 : value.meta) == null ? void 0 : _b.description) === "string"
    },
    {
      key: "meta.title",
      ok: typeof ((_c = value == null ? void 0 : value.meta) == null ? void 0 : _c.title) === "string"
    }
  ];
};
const isValidPostPageData = (value) => {
  return validatePost(value).every((v) => v.ok);
};
const getPostErrors = (value) => {
  validatePost(value).filter((v) => !v.ok).map((v) => v.key).join(", ");
};
async function readPostFile(postId) {
  ...
  try {
    ...
    const normalizedFile = fileContents.split("\n").filter((line) => !line.trim().startsWith("import ")).join("\n");
...
async function getPostIds() {
  return readdirSync(POSTS_DIRECTORY, { withFileTypes: true }).filter(
    (entry) => entry.isDirectory() && existsSync(path.resolve(POSTS_DIRECTORY, entry.name, "index.mdx"))
...
async function getPostsList() {
  ...
  return posts.sort(
    (a, b) => new Date(b.data.meta.date).getTime() - new Date(a.data.meta.date).getTime()
  );
}
async function main() {
